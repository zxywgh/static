...
function showHideIcp(){
    if(_icpDisplay=="1"){
        if(_icpLink==""){
            ...
            _icpLinkStr1="";
            var icpArray=_icpInfo.split("^");
            var icpLinkArray=_icpLink.split("^");
            for(var j=0;j<icpArray.length;j++){
                var icp=icpArray[j];
                if($.trim(icp)!=""){
                    var icpLink=icpLinkArray[j];
                    _icpLinkStr1=_icpLinkStr1+ (j!=0?'&nbsp;/&nbsp;':'&nbsp;&nbsp;') +"<a href='" +icpToTxt(icpLink) +"' target='_blank'>"+icpToTxt(icp)+"</a>";
                }
            }
            // _icpLinkStr1="<a href='" + icpToTxt(_icpLink) + "' target='_blank'>";
            _icpLinkStr1="<p>"+_icpLinkStr1;
            _icpLinkStr2="";
            if(_dueRamindDisplay=="1" && dueRemind != ""){
                _icpLinkStr2="&nbsp;/&nbsp;<span>"+dueRemind+"</span></p>";
            }else{
            	_icpLinkStr2="</p>";
            }
        }
        var _icpStr = _icpLinkStr1 + _icpLinkStr2;
...
function changeIcpUrlText(icp,icpUrl){
    _icpInfo = icpToTxt(icp);
    _icpLink = icpToTxt(icpUrl);
    showHideIcp();
}
...
//设置备案信息
function showIcpDialog(icp,icpUrl){
    var _showIcpDialog = $.dialog({
        id: 'showIcpDialog',
        title: $.i18n("template.login.icp"),
        url:_ctxPath + "/portal/loginTemplateController.do?method=getIcpPage",
        width:600,
        height:270,
        //htmlId:'showIcpDialogHtml',
        transParams:{
            "icp":icp,
            "icpUrl":icpUrl
        },
        buttons: [{
            ...
            handler: function () {
                ...
                //_showIcpDialog.close();
                var returnval=_showIcpDialog.getReturnValue();
                var formValidate=returnval.formValidate;
                if(formValidate){
                    changeIcpUrlText(returnval.icptext,returnval.icpUrltext);
                    $("#icp").val(returnval.icptext);
                    $("#icpUrl").val(returnval.icpUrltext);
                    _showIcpDialog.close();
                }
            }
        }, {
